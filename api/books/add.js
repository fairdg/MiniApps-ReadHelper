...
import { createDelivery } from '../../lib/repositories/deliveries.js'
...
export default async function handler(req, res) {
  ...
  const { telegramId, username, title, text, intervalMinutes } = req.body ?? {}
  ...
  try {
    ...
    const delivery = await createDelivery(book.id, intervalMinutes)
