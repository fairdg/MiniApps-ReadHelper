  ...
  <div class="sheet" :class="{ open }">
    ...
    <template v-if="admin">
      ...
      <p v-if="devMode" class="hint">
        На экране чтения появится кнопка "Отправить порцию сейчас" — для проверки доставки без
        ожидания реального интервала.
      </p>
    </template>
    ...
    <template v-if="owner">
      ...
      <p v-else class="hint">Пока нет вручную выданных Pro-подписок.</p>
      ...
      <p class="hint">
        Список не включает владельца с автоматическим Pro. Выдать подписку можно только тому, кто
        уже хоть раз открывал приложение.
      </p>
